...
export default function EquipmentList() {
  ...
  async function loadEquipment() {
    ...
    try {
      ...
      if (profilesResult.error) {
        console.error("Erro ao carregar utilizadores:", profilesResult.error);
      } else {
  ...
  const getEquipmentType = (item: Equipment) => {
    const value = `${item.name} ${item.model ?? ""}`.toLowerCase();
  ...
  const getTypeIcon = (item: Equipment) => {
  ...
  const getTypeLabel = (type: string) => {
    if (type === "smartphone") {
      return isPT ? "Smartphone" : "Smartphone";
    }
    ...
    return isPT ? "Computador" : "Computer";
  };
  ...
  const brands = useMemo(() => {
    return Array.from(
      new Set(equipment.map((item) => item.brands?.name).filter(Boolean)),
    ).sort();
  }, [equipment]);

  const types = useMemo(() => {
    return Array.from(new Set(equipment.map((item) => getEquipmentType(item))));
  }, [equipment]);
  ...
  const filteredEquipment = useMemo(() => {
    ...
    return equipment.filter((item) => {
      const matchesSearch =
        ...
        item.asset_tag?.toLowerCase().includes(normalizedSearch) ||
        item.brands?.name?.toLowerCase().includes(normalizedSearch) ||
        getProfileName(item.assigned_user)
          .toLowerCase()
      ...
      const matchesType =
        typeFilter === "all" || getEquipmentType(item) === typeFilter;

      const matchesBrand =
        brandFilter === "all" || item.brands?.name === brandFilter;

      return matchesSearch && matchesStatus && matchesType && matchesBrand;
    });
  }, [equipment, search, statusFilter, typeFilter, brandFilter, profiles]);
  ...
  return (
    <div className="min-h-full bg-[#080D1F] px-6 py-7 lg:px-8 text-white">
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        {isAdmin && (
          ...
          >
            ...
            {isPT ? "Adicionar equipamento" : "Add equipment"}
          </Link>
        )}
      </div>
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Package size={16} className="text-blue-400/70" />
          </div>

          <div className="mt-2 text-2xl font-bold">{totalEquipment}</div>

          <div className="mt-1 text-xs text-emerald-400">
            {isPT ? "Total registado" : "Total registered"}
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-2 text-2xl font-bold">{activeEquipment}</div>
        ...
        >
          ...
          <div className="mt-2 text-2xl font-bold">{maintenanceEquipment}</div>

          <div className="mt-1 text-xs text-amber-400">
            {isPT ? "Requer atenção" : "Requires attention"}
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-2 text-2xl font-bold">{criticalEquipment}</div>

          <div className="mt-1 text-xs text-red-400">
            {isPT ? "Descontinuados" : "Decommissioned"}
          </div>
        </div>
      </div>
      ...
      <div
        className="mb-4 rounded-xl border border-white/[0.06] bg-[#0D1730] p-4"
      >
        <div
        ...
        >
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={
                isPT ? "Pesquisar equipamento..." : "Search equipment..."
              }
          ...
          >
            <option value="all">{isPT ? "Estado" : "Status"}</option>
            ...
            <option value="inactive">{isPT ? "Inativo" : "Inactive"}</option>

            <option value="decommissioned">
              {isPT ? "Descontinuado" : "Decommissioned"}
            </option>
          </select>
          ...
          <select
            ...
            className="h-11 rounded-lg border border-white/[0.07] bg-[#080D1F] px-3 text-sm text-white/60 outline-none focus:border-blue-500/40"
          >
            <option value="all">{isPT ? "Tipo" : "Type"}</option>
          ...
          <select
            ...
            className="h-11 rounded-lg border border-white/[0.07] bg-[#080D1F] px-3 text-sm text-white/60 outline-none focus:border-blue-500/40"
          >
            <option value="all">{isPT ? "Marca" : "Brand"}</option>
      ...
      <div
        className="overflow-hidden rounded-xl border border-white/[0.06] bg-[#0D1730]"
      >
        {/* TABLE HEADER */}
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-white">
                {isPT ? "Lista de equipamentos" : "Equipment list"}
              </h2>

              <p className="mt-0.5 text-xs text-white/35">
                {filteredEquipment.length}{" "}
                {isPT ? "equipamentos encontrados" : "equipment found"}
              </p>
            </div>
            ...
            >
              ...
              {isPT ? "Filtros aplicados" : "Filters available"}
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          <div
            className="flex min-h-[300px] items-center justify-center text-sm text-white/35"
          >
          ...
          >
            <div
              className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-white/[0.04] text-white/25"
            >
              <Package size={18} />
            </div>

            <h3 className="text-sm font-semibold text-white/70">
              {isPT ? "Não existem equipamentos" : "No equipment found"}
            </h3>
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px]">
                <thead>
                  <tr
                    className="border-b border-white/[0.05] text-left"
                  >
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-500/[0.08] text-blue-400"
                          >
                            {getTypeIcon(item)}
                          </div>
                      ...
                      <td className="px-5 py-4">
                        <span
                          className="text-sm font-mono text-white/45"
                        >
                          {item.asset_tag ?? `IT-${item.id}`}
                        </span>
                      </td>
            ...
            >
              ...
              <div className="flex items-center gap-1">
                ...
                <div
                  className="flex h-8 min-w-8 items-center justify-center rounded-md bg-blue-600 px-2 text-xs font-medium text-white"
                >
                  {safeCurrentPage}
                </div>
